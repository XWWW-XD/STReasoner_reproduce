...
- 运行命令：`python repro_kaggle/00_smoke_test_scripts/00_check_kaggle_env.py`
...
- 运行命令：`bash repro_kaggle/00_smoke_test_scripts/01_setup_kaggle_t4.sh`
...
- 成功后下一步：运行 `python repro_kaggle/00_smoke_test_scripts/02_inspect_stbench.py`。

### 02_inspect_stbench.py

- 作用：检查 `Time-HD-Anonymous/ST-Bench` 的各个 subset 是否能直接读取，并预览第一条样本字段。
- 运行命令：`python repro_kaggle/00_smoke_test_scripts/02_inspect_stbench.py`
- 它检查哪些 subset：`ST-Test`、`ST-Test-Text`、`ST-SFT`、`ST-CoT`、`ST-Align`。
- 输出文件：`repro_kaggle/outputs/early_smoke_tests/stbench_inspect.log`
...
- 运行命令：`python repro_kaggle/00_smoke_test_scripts/03_load_streasoner_smoke.py --model_name Time-HD-Anonymous/STReasoner-8B --load_in_4bit true --attn_backend sdpa`
...
- 运行命令：`python repro_kaggle/00_smoke_test_scripts/04_run_one_sttest_sample.py --model_name Time-HD-Anonymous/STReasoner-8B --load_in_4bit true --attn_backend sdpa --index 0`
- 输出文件：`repro_kaggle/outputs/early_smoke_tests/one_sttest_prediction.json` 和 `repro_kaggle/outputs/early_smoke_tests/run_one_sttest_sample.log`
- 成功标志：日志里出现 `ONE_SAMPLE_RUN_PASS`，JSON 里有 `prediction`。

### 05_eval_sttest_tiny.py

- 作用：在 `ST-Test` 上跑 5-20 条小规模评测，快速观察输出格式、解析成功率和粗略正确率。
- 运行命令：`python repro_kaggle/00_smoke_test_scripts/05_eval_sttest_tiny.py --model_name Time-HD-Anonymous/STReasoner-8B --max_samples 5`
- 输出文件：`repro_kaggle/outputs/early_smoke_tests/sttest_tiny_predictions.jsonl`、`repro_kaggle/outputs/early_smoke_tests/sttest_tiny_summary.json`、`repro_kaggle/outputs/early_smoke_tests/sttest_tiny_eval.log`
