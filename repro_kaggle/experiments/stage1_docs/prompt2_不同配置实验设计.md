请按照下面的实验设计，执行 STReasoner 阶段一的实验一：不同精度推理资源测试。  
...
实验前请先完成两件证据查找，并把证据位置写入 experiment1_summary.md；如果证据影响实验配置，再暂停报告。 
1. 请找出作者代码中选择的精度的证据。  
- 训练脚本中的精度设置  
- 推理脚本中的精度设置  
- 如果推理代码没有显式设置 dtype，请说明实际 dtype 需要如何确认  
2. 请找出作者设置的 max_new_tokens / max_tokens 为多少，证据在哪。  
- 注意作者如果使用 vLLM，参数可能叫 max_tokens  
- 如果我用 Hugging Face generate，对应记录为 max_new_tokens  
- Hugging Face 文档中 max_new_tokens 表示最多生成的新 token 数，不包含 prompt tokens
...
报告要求补充：
（1）实验名称统一叫“实验一”，不要再额外使用 tiny20/main20 等作为实验名。样本只作为三组测试样例：
1. 主测试样例：4类任务×每类5条=20条
2. 论文样例
3. 压力测试样例
...
（3）总表中不展开三组样例，表项如下，列好表后先让我确认表体是否正确。再填入数值。

（3）每个配置下面再写详细报告，并在详细报告中分别记录：
- 主测试样例结果
- 论文样例结果
- 压力测试结果

最终报告路径：
repro_kaggle/experiments/stage1_docs/experiment_summary.md
...
| :------------: | :-----------------------------: |
|       模型       |          STReasoner_8B          |
|       样本       | tiny20 + paper cases + stress 1 |
|   batch size   |                1                |
| max_new_tokens |               512               |
