...
SCRIPT_PATH = PROJECT_ROOT / "repro_kaggle/experiments/scripts/stage1_script/run_experiment1_precision_resource2.py"
...
CONFIGS: dict[str, ConfigSpec] = {
    "4bit_single": ConfigSpec("4bit_single", "4bit单卡", "4bit", "1", "single"),
    "8bit_single": ConfigSpec("8bit_single", "8bit单卡", "8bit", "1", "single"),
    "fp16_single": ConfigSpec("fp16_single", "fp16单卡", "fp16", "1", "single"),
...
"""
    (DOC_ROOT / "experiment_summary2.md").write_text(summary, encoding="utf-8")
...
- 失败阶段与瓶颈类型
"""
        (DOC_ROOT / f"experiment1_{config_name}2.md").write_text(detail, encoding="utf-8")
...
def run_config(config_name: str) -> int:
    ...
    logger = TeeLogger(out_dir / "run2.log")
    records_by_group: dict[str, list[dict[str, Any]]] = {"main": [], "paper": [], "stress": []}
    data_groups = {
        "main": (MAIN_DATA, out_dir / "main_predictions2.jsonl"),
        "paper": (PAPER_DATA, out_dir / "paper_predictions2.jsonl"),
        "stress": (STRESS_DATA, out_dir / "stress_predictions2.jsonl"),
    }
    ...
    try:
        ...
        write_json(out_dir / "summary2.json", summary)
...
"""
    (DOC_ROOT / f"experiment1_{spec.name}2.md").write_text(report, encoding="utf-8")


def write_filled_summary_if_possible() -> None:
    summaries: dict[str, dict[str, Any]] = {}
    for config_name in CONFIGS:
        path = RESULT_ROOT / config_name / "summary2.json"
        if path.exists():
...
详见各配置报告与 `summary2.json` 中的 `bottleneck_counts`。失败原因统一归入资源瓶颈、输入/生成瓶颈、输出与评测瓶颈三类；确定的原因尽量细写，不确定时只写粗略阶段和错误信息。
"""
    (DOC_ROOT / "experiment_summary2.md").write_text(summary_doc, encoding="utf-8")
...
def main() -> int:
    ...
    if command == "prepare":
        write_prepare_docs()
        print(f"已生成空表和证据文档: {DOC_ROOT / 'experiment_summary2.md'}")
        return 0
