...
**Stage1 四精度实验**（`run_experiment1_new_version.py`、`08_prepare_stage1_exp1_subsets.py`、`exp1_resource_tiny20`）**已删除**；历史结论见 `stage1_docs/`。
...
- **现行脚本**：`stage2_2_run_paper_cases.py`、`stage2_4_graph_ablation_paper_cases.py`（`stage2_run_smarttest.py` 已删）。
- **数据/结果**：`stage2_subsets/`、`experiments/results/` 等仍可能被报告引用。

| 阶段 | 脚本 | 数据 | max_new_tokens | 输出默认目录 |
|------|------|------|----------------|--------------|
| **Stage 2.2 Paper Cases** | `experiments/scripts/stage2_2_run_paper_cases.py` | `PaperCases.jsonl` 4 条 | 6144 | `stage2_2_paper_cases/` |
| **Stage 2.4 graph 消融** | `stage2_4_graph_ablation_paper_cases.py` | 同上 | 6144 | `results/stage2.4_*` |
...
```mermaid
flowchart TB
  FULL["ST-Test 全量\n四类 jsonl"] --> PC["paper_cases\n4条\nPaperCases.jsonl"]
  PC --> S22["Stage2.2 / 2.4 A100"]
  FULL --> VLLM["官方 vLLM 全量推理"]
```

| 子集 | 条数 | 路径 | 用途 |
|------|------|------|------|
| paper_cases | 4 | `repro_autodl/experiments/stage2_subsets/paper_cases/PaperCases.jsonl` | 论文 Appendix 匹配样例（现行） |
| ST-Test 全量 | — | `data/ST-Bench/ST-Test/` | 正式 benchmark |

paper_cases 已固化在 `stage2_subsets/paper_cases/`（原 `exp1_resource_tiny20/paper_cases` 与 `08_prepare` 已删除）。

### 5.2 复现层完整数据流

```mermaid
flowchart TB
  HF["HF ST-Bench ST-Test"] --> PC["PaperCases.jsonl 固化"]
...
|------|-----------|-------------------|
| 入口 | `inference/inference_tsmllm_vllm.py` | `stage2_2_run_paper_cases.py` 等 |
| 引擎 | vLLM-TS + 多 GPU 副本 | HF fp16 单卡（AutoDL） |
| 默认 max_tokens | 512（README 默认） | 6144（paper_cases / 正式 ST-Test） |
| 硬件假设 | 8× A100-80GB | AutoDL 单 A100 |
| 输出格式 | `generated_answer.json` | jsonl（`response` 字段） |
| 数据规模 | 全量 ST-Test 四类 | paper_cases 4 条 |
| 目的 | 论文复现、正式 benchmark | 论文样例回归 / 链路排查；评分走 `evaluate_qa.py` |
...
| Stage2.2 Paper Cases | 多轮调试 | `2.2_consequences/`；数据 `PaperCases.jsonl` |
...
- paper_cases 不能替代完整 ST-Test 验证论文整体效果
...
| paper_cases 数据 | `00_new_codes/repro_autodl/experiments/stage2_subsets/paper_cases/PaperCases.jsonl` |
...
| 复现 Stage 2.2 / 2.4 | AutoDL fp16 paper_cases 小样本 |
