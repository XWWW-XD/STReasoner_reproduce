...
OUTPUTS_JSONL = ROOT / "00_new_codes/reports/artifacts/sttest_full_6144_outputs_with_gold.jsonl"
MISMATCH_JSON = OUT_DIR / "task_level_reconstruction_mismatch.json"

TASKS = ["forecasting", "correlation", "entity", "etiological"]
TASK_FROM_JSONL = {
    "reasoning_forecasting": "forecasting",
    "reasoning_correlation": "correlation",
    "reasoning_entity": "entity",
    "reasoning_etiological": "etiological",
}
...
def load_outputs_by_task() -> dict[str, list[dict]]:
    ...
    for line in OUTPUTS_JSONL.read_text(encoding="utf-8").splitlines():
        if not line.strip():
            continue
        row = json.loads(line)
        task = TASK_FROM_JSONL.get(row["task"])
        if task:
...
def audit_mention_patterns(sample_per_task: int = 40) -> str:
    ...
    lines = [
        ...
        "",
        f"Source: `{OUTPUTS_JSONL.relative_to(ROOT)}`",
        "",
    ]
    for task in TASKS:
...
def export_mismatch_sample_cards() -> dict:
    ...
    jsonl_rows: dict[tuple[str, int], dict] = {}
    for line in OUTPUTS_JSONL.read_text(encoding="utf-8").splitlines():
        if not line.strip():
            continue
        row = json.loads(line)
        task = TASK_FROM_JSONL.get(row["task"])
        if task:
            jsonl_rows[(task, int(row["idx"]))] = row

    cards = []
    for task, idx in cards_spec:
        row = jsonl_rows.get((task, idx))
        windows = []
        cross_extra = None
        if task == "forecasting":
        ...
        cards.append(
            {
                ...
                "outputs_jsonl": "00_new_codes/reports/artifacts/sttest_full_6144_outputs_with_gold.jsonl",
